...
static void	draw_minimap_cells(t_data *data, int start_x, int start_y,
	int cell_size)
{
	int	i;
	int	j;
	...
	while (++i < data->minimap.visible_cells)
	{
		...
		while (++j < data->minimap.visible_cells)
		{
			...
			ft_draw_colored_cell(data, j * cell_size, i * cell_size, color);
		}
	}
}
...
void	ft_draw_minimap(t_data *data)
{
	...
	ft_draw_colored_cell(data, half * cell_size, half * cell_size,
		RED_PIXEL);
